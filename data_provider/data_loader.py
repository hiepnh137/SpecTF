...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            cols_data = df_raw.columns[self.args.feat_start_id:self.args.feat_end_id]
            df_data = df_raw[cols_data]
            print('cols_data: ', cols_data.shape)
        ...
        # print('df_raw:'  , df_raw[text_name])
        raw_text = df_raw[text_name].tolist()
        # print('self.text:' , self.text)
        for i in range(len(raw_text)):
            if pd.isnull(raw_text[i]):
            # if True:
                raw_text[i] = 'No information available'
        df_raw[text_name] = raw_text

        self.text = df_raw[[text_name]][border1:border2]
    ...
    def get_text(self, indices):
        ...
        text=np.array([self.text[s_begin:s_end] for s_begin, s_end in zip(s_begins, s_ends)])
        return text
    
    def get_text_embedding(self, indices):
        if isinstance(indices, torch.Tensor):
            indices = indices.numpy()

        s_begins = indices
        s_ends = s_begins + self.seq_len
        # print('text_embeddings: ', self.text_embeddings.shape)
        text_embeddings=np.array([self.text_embeddings[s_begin:s_end] for (s_begin, s_end) in zip(s_begins, s_ends)])
