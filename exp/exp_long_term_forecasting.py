...
# Add this function to calculate and print MACs
def print_model_macs_and_params(model, input_size):
    with torch.cuda.device(0):  # Ensure the calculation is done on the correct device
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        # print('Number of trainable parameters of Text Encoder: ', self.count_parameters(self.mlp))
        # print('Number of trainable parameters of frequency model: ', self.count_parameters(self.model))
        # print("Text Encoder:")
        # print_model_macs_and_params(self.mlp, (configs.seq_len, configs.llm_dim))

        # # Print MACs and parameters for the frequency model
        # print("Frequency Model:")
        # print_model_macs_and_params(self.model, (configs.seq_len, configs.enc_in))
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                end_lm_time = time.time()
                print(f"Time taken for LLM processing: {end_lm_time - start_lm_time:.4f} seconds")
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                # encoder - decoder
                ...
                print(f"Time taken for model processing: {end_model_time - start_model_time:.4f} seconds")
                if self.Doc2Vec==False:
                    if self.pool_type=="avg":                
                        global_avg_pool = F.adaptive_avg_pool1d(prompt_emb.transpose(1, 2), 1).squeeze(2)
                        prompt_emb=global_avg_pool.unsqueeze(-1)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(test_loader):
                ...
                # print(f"Time taken for LLM processing: {end_llm_time - start_llm_time:.4f} seconds")
        ...
        print('test time:', np.mean(time_list))
